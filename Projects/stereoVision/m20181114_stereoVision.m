function m20181114_stereoVision()
...
function img = generateDisparityMap(img,stereoParams)
    ...
    img.dispRange = 16*(6+1)+16*6*[-1,1];           % close and long range
...
%     % Filter disparity map
    img.dispMap(img.dispMap < img.dispRange(1)+5) = NaN;      % explicitly remove data smaller than minimum disparity
    img.dispMap(img.dispMap > img.dispRange(2)-5) = NaN;      % explicitly remove data larger than maximum disparity
...
function plotStuff(img,count)
    ...
    try         
        figure(10);
        hAxes = pcshow(img.ptCloudScene,'VerticalAxis','Y','VerticalAxisDir','Up','MarkerSize',100);
...
%         xlim(zoom*[-3,3]);          % left - right
%         ylim(zoom*[-2,2]);          % down - up
%         zlim(zoom*[0,10]);               % forward
        drawnow('limitrate');
